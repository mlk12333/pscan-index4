...
void Graph::read_graph(){
    ...
    while (!infile.eof())            // 若未到文件结束一直循环
    {
        ...
        out_edges_search[u][v] = v;
//        in_edges_search[v][u] = u;
...
vector<int> Graph::get_f_and_c_num(int a,int b, int c){
    ...
    if(out_edges_search[a].find(b) != out_edges_search[a].end()) a_to_b = 1;
    if(out_edges_search[b].find(a) != out_edges_search[b].end()) b_to_a = 1;
    if(out_edges_search[a].find(c) != out_edges_search[a].end()) a_to_c = 1;
    if(out_edges_search[c].find(a) != out_edges_search[c].end()) c_to_a = 1;
    if(out_edges_search[c].find(b) != out_edges_search[c].end()) c_to_b = 1;
    if(out_edges_search[b].find(c) != out_edges_search[b].end()) b_to_c = 1;
//
